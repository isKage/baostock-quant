...
from utils.stock_name import stock_name, stock_code
...
def individual_byname(request, name='股票名称'):
    # 获取当前日期
    current_date = datetime.now().strftime('%Y-%m-%d')

    if request.method == 'POST':
        name = request.POST.get('name', '')  # 获取股票名称
        date = request.POST.get('date', '')  # 获取开始日期

        # 查询股票名称
        stockcode = stock_code(name)  # 获取股票代码
    ...
    return render(request, "individual_byname.html", {'name': name})
